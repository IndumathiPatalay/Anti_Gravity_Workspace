...
const Register: React.FC = () => {
        ...
        if (clientId) {
          try {
            ...
            g.accounts.id.renderButton(
              document.getElementById('google-signin-btn-real-reg'),
  ...
  const handleGoogleCredentialResponse = async (response: any) => {
    ...
    try {
      ...
      if (axiosError.response) {
        setError(axiosError.response.data?.error || 'Google signup failed.');
      } else {
  ...
  const handleMockGoogleLogin = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const axiosError = err as { response?: { data?: { error?: string } }; request?: unknown };
      if (axiosError.response) {
        setError(axiosError.response.data?.error || 'Mock signup failed.');
      } else {
  ...
  return (
    <div className="center-screen" style={{ padding: '2rem 1rem' }}>
      <div className="glass-panel animate-in" style={{ width: '100%', maxWidth: '750px', padding: '2.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', alignItems: 'center', width: '100%', marginBottom: '1.5rem' }}>
          {clientId && googleAvailable && (
            <div id="google-signin-btn-real-reg" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}></div>
          )}
